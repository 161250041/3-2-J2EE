...
<script>
  ...
  export default {
    ...
    data(){
      return{
        id:parseInt(this.$route.params.code),
        time: [mytime, mytime],
        list1:[],
        list2:[],
        totalPrice:0,
    ...
    methods:{
      getList(){
        // alert(this.time);
        this.$axios({
          url: 'http://localhost:8081/statistics/getRestStatistics',//请求的地址
          method: 'post',//请求的方式
          data:{
            id:this.id,
          ...
          for (var i=0;i< mylist.length;i++) {
            ...
            if (mylist[i].state === "成功") {
              this.stateData[0].value = this.stateData[0].value + 1;
            }else if (mylist[i].state === "已退款") {
              this.stateData[1].value = this.stateData[1].value + 1;
            }else {
              this.stateData[2].value = this.stateData[2].value + 1;
            }
          }
